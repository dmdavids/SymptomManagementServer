	...
	public void createDatabaseForTesting() throws Exception {
		...
		for(int i=0; i < 50; i++) {
			...
			for(Medication med: harryPotter.getPrescriptions()) 
				TestData.addMedLogToPatient(TestData.randomMedLog(med), harryPotter);
		}
		harryPotter = smController.updatePatient(harryPotter.getId(), harryPotter);
		
		for(int i=0; i < 50; i++) {
			TestData.addPainLogToPatient(TestData.randomPainLog(), hanSolo);
			for(Medication med: hanSolo.getPrescriptions()) 
				TestData.addMedLogToPatient(TestData.randomMedLog(med), hanSolo);
		}
		...
		for(int i=0; i < 50; i++) {
			...
			for(Medication med: bellaSwan.getPrescriptions()) 
				TestData.addMedLogToPatient(TestData.randomMedLog(med), bellaSwan);
		}
		bellaSwan = smController.updatePatient(bellaSwan.getId(), bellaSwan);
