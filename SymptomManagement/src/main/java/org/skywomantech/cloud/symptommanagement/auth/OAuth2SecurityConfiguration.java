...
public class OAuth2SecurityConfiguration {
	...
	protected static class ResourceServer extends
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			http
			.authorizeRequests()
				.antMatchers("/oauth/token").anonymous();  // anyone can do this... pre-login stuff
			
			
			// If you were going to reuse this class in another
			// application, this is one of the key sections that you
			// would want to change
			// NOTE to self ... this is probably a top down setup
			// so most restrictive at the top then least restrictive
			// at the bottom 
//			http
//			.authorizeRequests()
//				.antMatchers("/**").permitAll();  // just let everyone have full access until I get it figured out
//			
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			// NOTE to self ... CLIENT == APP identifier (how trusted is the APP not the user!)
			
			// Create a client that has "read" and "write" access to the
	        // video service
			.withClient("mobile").authorizedGrantTypes("password")
			.authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
			.scopes("read","write").resourceIds("patient", "physician","medication","alert")
			.and()
			// Create a second client that only has "read" access to the
			// video service
			.withClient("mobileReader").authorizedGrantTypes("password")
			.authorities("ROLE_CLIENT")
			.scopes("read").resourceIds("patient", "physician","medication","alert")

					.accessTokenValiditySeconds(2419200).and().build();  // 2419200 is remember-me value of 4 weeks

			// NOTe to self ... NOW we are talking users!
			
			// Create a series of hard-coded users. 
			
			//NOTE to self... can create a persisted database with a class that implements UserDetailsManager
