...
public class OAuth2SecurityConfiguration {
	...
	/**
	 * Configure who is allowed to access which parts of the resource server
	 */
	@Configuration
	...
	protected static class ResourceServer extends
		...
		// This method configures the OAuth scopes required by CLIENTS aka apps
		// to access
		// all of the paths in the service ... this is not configuring USERS
		@Override
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			// NOTE to self ... CLIENT == APP identifier (how trusted is
			// the APP not the USER!)

			// Create a client that has "read" and "write" access to the service
			// which is the app that I am creating
			.withClient("mobile")
			.authorizedGrantTypes("password")
			.authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
			.scopes("read", "write")
			.resourceIds("patient", "physician", "medication", "alert")
			.accessTokenValiditySeconds(2419200)  // this is set for 4 weeks 
			.and()
			.build(); 

			// NOTe to self ... NOW we are talking users!

			// Create a series of hard-coded users.

			// NOTE to self... can create a persisted database with a class that
			// implements UserDetailsManager
			// this one just stores a Map in memory
...
//							User.create("user5", "pass", "USER")));
			...
			// id/secret, when sending a request for a password grant, we make each client
			// a user as well. When the BASIC authentication information is pulled from
			// the request, this combined UserDetailsService will authenticate that
			// the client is a valid "user".
